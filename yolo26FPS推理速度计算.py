# @Time       : 2026/2/28 15:07
# @File       : yolo26FPS推理速度计算.py
# @Description:
import warnings
...
def AiGuai_get_weight_size(path):
    """获取模型权重文件的大小（以MB为单位）。."""
    try:
        stats = os.stat(path)
...
def AiGuai_warmup_model(model, device, example_inputs, iterations=200):
    """模型预热，准备进行高效推理。."""
...
def AiGuai_test_model_latency(model, device, example_inputs, iterations=1000):
    """测试模型的推理延迟。."""
...
def main(opt):
    ...
    model, _ = load_checkpoint(weights, device=device, fuse=True)
    ...
    AiGuai_warmup_model(model, device, example_inputs, opt.warmup)
    mean_latency, std_latency = AiGuai_test_model_latency(model, device, example_inputs, opt.testtime)

    logging.info(
        f"Model weights: {opt.weights} Size: {AiGuai_get_weight_size(opt.weights)}M "
...
if __name__ == "__main__":
    # 使用说明：只需要修改'--weights'参数，替换成自己训练好的模型权重best.pt文件
    model_weight_path = r"D:\PycharmProjects\ultralytics\yolo26n.pt"
    parser = argparse.ArgumentParser(description="Test YOLO model performance.")
    parser.add_argument("--weights", type=str, default=model_weight_path, help="trained weights path")
